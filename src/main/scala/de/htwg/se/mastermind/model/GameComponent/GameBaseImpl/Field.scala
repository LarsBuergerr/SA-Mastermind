...
case class Field(var matrix: Matrix[Option[Stone]], var hmatrix: Matrix[Option[HStone]]):
  ...
  def cells(row: Int = 0, cellWidth: Int = 3, cellCount: Int = this.cols): String = {
    matrix.row(row).map(_.getOrElse(" ").toString).map(" " * ((cellWidth - 1) / 2) + _ + " " * ((cellWidth - 1) / 2)).mkString("|", "|", "|") +
      space * 3 + hmatrix.row(row).map(_.getOrElse(" ").toString).map(" " + _ + " ").mkString("[", "|", "]") + eol
  }
  ...
  def placeGuessAndHints(stone: Vector[Option[Stone]])(hints: Vector[Option[HStone]])(row: Int) = copy(matrix.replaceRow(row, stone), hmatrix.replaceRow(row, hints))
