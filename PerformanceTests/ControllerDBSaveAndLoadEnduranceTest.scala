...
class ControllerDBSaveAndLoadEnduranceTest extends Simulation {
  ...
  private val scn = scenario("ControllerDBSaveAndLoadEnduranceTest")
	    .exec(
      http("request_GoToControllerAPI")
        .get("/")
        .headers(headers_0)
    )
      // TODO - Run crashed ... repeat führt zu fehler .. kp warum...
	 /*
	.repeat(50) {
		.exec(
			http("request_GoToControllerAPI")
